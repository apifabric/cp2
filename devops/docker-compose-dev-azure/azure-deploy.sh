...
resourcename="cp2"  # lower case, only
resourcegroup="cp2_rg"
...
# git clone https://github.com/apilogicserver/cp2.git
# cd cp2
...
az group create --name cp2_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group cp2_rg --sku S1 --is-linux
az container create --resource-group cp2_rg --name cp2 --image apilogicserver/cp2:latest --dns-name-label cp2 --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//cp2.westus.azurecontainer.io:5656'
